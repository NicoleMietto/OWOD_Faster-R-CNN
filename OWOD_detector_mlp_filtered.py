    ...
    def forward(self, images, targets=None, dino_features_list=None):
        if self.training:
            ...
            for i in range(len(images)):
                ...
                # --- NOVITÀ: FILTRO DIMENSIONE (Minimo 32x32 pixel) ---
                if len(raw_unknowns) > 0:
                    widths = raw_unknowns[:, 2] - raw_unknowns[:, 0]
                    heights = raw_unknowns[:, 3] - raw_unknowns[:, 1]
                    min_size_mask = (widths >= 32) & (heights >= 32)
                    raw_unknowns = raw_unknowns[min_size_mask]
                    unk_scores = unk_scores[min_size_mask]
                ...
                if len(raw_unknowns) > 0:
                    ...
                    top_unknowns = torch.empty((0, 4), device=raw_unknowns.device)
                    top_unk_scores = torch.empty((0,), device=raw_unknowns.device)
