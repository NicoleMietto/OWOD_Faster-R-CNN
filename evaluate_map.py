...
def evaluate_map(checkpoint_path, val_json_path, image_dir, device, use_spatial_cnn=False):
    print(f"\n--- CALCOLO mAP UFFICIALE: {checkpoint_path} ---")
    ...
    # PER IL REPORT FINALE USA TUTTE LE IMMAGINI (Circa 5000)
    images_to_eval = valid_images
    
    # Inizializza la metrica di torchmetrics
    ...
    for img_info in tqdm(images_to_eval):
        ...
        # Filtriamo le predizioni "Unknown" perché la mAP la calcoliamo SOLO sulle classi note
        mask_known = (pred_labels != 81) & (pred_labels != 11) & (pred_labels != 21)
    ...
    print("\nCalcolo finale mAP in corso (può richiedere qualche minuto)...")
